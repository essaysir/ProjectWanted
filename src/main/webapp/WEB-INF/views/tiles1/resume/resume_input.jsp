...
<script type="text/javascript">
		...
		$(document).ready(function(){
			var fk_userid = '${mvo.userid}' ;
			console.log(fk_userid);
/* 			
 			$("textarea[name='uploadLink']").on('change' , function(){
				console.log(" 확인용 uploadLink val"+$("textarea[name='uploadLink']").val());
				console.log(" 확인용 uploadLink text"+$("textarea[name='uploadLink']").text());
			});
*/
		...
		///////////////////////////////////CONTROLLER 로 데이터 보내기////////////////////////////////////////
         ...
         // 커리어 부분 나타내는 Li들 선택
         const careerElements = document.querySelectorAll('li[id^="careerDel_range"]');
         
         careerElements.forEach((careerElement) => {

            // 커리어 요소에 해당하는 input들 값 가져오기
            const startCareerDate = careerElement.querySelector('input[name="start_careerDate"]').value;
            const endCareerDate = careerElement.querySelector('input[name="end_careerDate"]').value;
            const company = careerElement.querySelector('input[name="company"]').value;
            const department = careerElement.querySelector('input[name="department"]').value;

            // 커리어의 하위 li들(performance) 선택 
            const performanceElements = careerElement.querySelectorAll('li[id^="performanceDel_range"]');

            // 퍼포먼스 담길 배열
            const performancevoList = [];

            // 퍼포먼스 요소들 반복하기
            performanceElements.forEach((performanceElement) => {
              // 퍼포먼스의 요소에 해당하는 input 밸류들 가져오기
              const performance = performanceElement.querySelector('input[name="performance"]').value;
              const startDateDetail = performanceElement.querySelector('input[name="start_date_detail"]').value;
              const endDateDetail = performanceElement.querySelector('input[name="end_date_detail"]').value;
              const performContent = performanceElement.querySelector('textarea[name="perform_content"]').value;

              // 퍼포먼스 오브젝트 만들기
              const performanceObj = {
                performance: performance,
                start_date: startDateDetail,
                end_date: endDateDetail,
                perfrom_content: performContent
              };

              // 퍼포먼스 배열에 퍼포먼스 객체 넣어주기
              performancevoList.push(performanceObj);
            });

            // 커리어 객체 만들기
            const career = {
              start_date: startCareerDate,
              end_date: endCareerDate,
              company: company,
              department: department,
              performancevoList: performancevoList
            };
		...
		function addPerformance(count) {
		  ...
		  var html = '<li id="performanceDel_range'+performanceDel_count+'">\n' +
				     ...
				     '	<input type="hidden" name="start_date_detail" class="hiddenYear3'+performanceDel_count+'  date'+performanceDel_count+'  " value="">' +	
				     '  &nbsp;- &nbsp;\n' +
				     ...
				     '	<input type="hidden" name="end_date_detail" class="hiddenYear4'+performanceDel_count+' date'+performanceDel_count+' " value="">' +	
...
<form name="resumeFrm" id="resumeForm" method="post">
		...
		<div class="container my-5">
				...
				<ul id="addCareer"></ul>
					...
					<!--  스킬  시작 -->
					<div class="resume-header" style="margin-top:60px; margin-bottom: 20px;">
						스킬
					</div>
					...
					<!--  적용하기 클릭 후 선택했던 값들이 들어가는 공간 -->                         
					 <div class="data-result" style="margin-top:20px;">
